...
namespace ShaderEditor
{
	public class ShaderUtils
	{
		...
		public static void DisassembleShader(byte[] inShader, ref string outShader)
		{
            ...
            // We use TrimEnd here to prevent the NUL character from being generated at the end of .asm files
			// Not entirely sure how safe this is, but it seems to do the job
            outShader = System.Text.Encoding.Default.GetString(outShaderAsByte).TrimEnd('\0');
		}
		...
		public static int BuildComments(ref byte[] outShader, Structures.SHADER_VARIABLE[] shader_var, Structures.VARIABLE[] globalVariables, Structures.VARIABLE[] localVariables)
		{
			...
			if (shader.comments.shaderType != 65535)
            {
                shader.comments.profile = $"vs_{shader.comments.version.version2}_{shader.comments.version.version1}";
            }
			else
            {
                shader.comments.profile = $"ps_{shader.comments.version.version2}_{shader.comments.version.version1}";
            }
        ...
        Xenon's priority is less than on PC, so the check is not ideal
		...
		*/
        {
			...
			for (int a = 0; a < fxc.vertexFragmentCount; a++)
            {
                ...
                {
					if (fxc.vertexFragment[0].shader[0] == 0x0 && fxc.vertexFragment[0].shader[1] == 0x3 && fxc.vertexFragment[0].shader[2] == 0xFE && fxc.vertexFragment[0].shader[3] == 0xFF)
                    {
						isPC++;
                    }
					else
                    {
					    if (fxc.vertexFragment[0].shader[0] == 0x10 && fxc.vertexFragment[0].shader[1] == 0x2A && fxc.vertexFragment[0].shader[2] == 0x11 && fxc.vertexFragment[0].shader[3] == 0x01)
                        {
			...
			for (int a = 0; a < fxc.pixelFragmentCount; a++)
            {
                ...
                {
					if (fxc.pixelFragment[0].shader[0] == 0x0 && fxc.pixelFragment[0].shader[1] == 0x3 && fxc.pixelFragment[0].shader[2] == 0xFF && fxc.pixelFragment[0].shader[3] == 0xFF)
                    {
						isPC++;
                    }
					else
                    {
					    if (fxc.pixelFragment[0].shader[0] == 0x10 && fxc.pixelFragment[0].shader[1] == 0x2A && fxc.pixelFragment[0].shader[2] == 0x11 && fxc.pixelFragment[0].shader[3] == 0x00)
                        {
						    throw new Exception("Unknown Shader");
                        }
                    }
